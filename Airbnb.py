...
st.set_page_config(page_title= "Airbnb Data Visualization",
                   ...
                   menu_items={'About': """# This dashboard app user friendly
                                        Data has been gathered from mongodb atlas"""}
                  )

with st.sidebar:
    selected = option_menu("Menu", ["Home","Overview","Explore","Review","Tableau"], 
                           icons=["house","graph-up-arrow","bar-chart-line","star","speedometer2"],
                           menu_icon= "menu-button-wide",
                           default_index=0,
                           styles={"nav-link": {"font-size": "20px", "text-align": "left", "margin": "-2px", "--hover-color": "#fa5aff"},
                                   "nav-link-selected": {"background-color": "#5adeff"}}
                          )
...
if selected == "Home":
    ...
    col1.markdown("## :blue[About] :  Airbnb is an American San Francisco-based company operating an online marketplace for short- and long-term homestays and experiences. The company acts as a broker and charges a commission from each booking. The company was founded in 2008 by Brian Chesky, Nathan Blecharczyk, and Joe Gebbia. Airbnb is a shortened version of its original name, AirBedandBreakfast.com. Airbnb is the most well-known company for short-term housing rentals.")
...
# REVIEW PAGE      
if selected == "Review":
    st.markdown("## Reviews based on room type")

    rev_df = df.groupby('Room_type',as_index=False)['Review_scores'].mean().sort_values(by='Review_scores')
    fig = px.bar(data_frame=rev_df,x='Room_type',y='Review_scores',color='Review_scores',text='Review_scores')
    st.plotly_chart(fig,use_container_width=True)
    st.balloons()

# Tableau PAGE      
if selected == "Tableau":
    st.markdown("## Click the link tableau dashboad view will open")
    tableau_dashboard_url = "https://public.tableau.com/app/profile/aravinth.ramesh8737/viz/Airbnb_17028350777250/airbnbdataanalysis#1"
    st.balloons()
    st.markdown(f"[Link to Tableau Dashboard]({tableau_dashboard_url})")
    st.markdown(" ")
    st.markdown(" ")
    st.markdown("### Tableau Dashboard image")
    st.image("Airbnb dashboard.png")
